...
#         fieldnames = ["Pothole ID", "X-coordinate", "Y-coordinate", "Confidence Score", "Area", "Width", "Height", "Area_Classification", "Depth_mono", "Pixel Distance", "Scale Factor", "Depth_Classification", "Actual_Depth"]
...
#                     pothole_depth = estimate_depth_for_potholes_in_frame(frame, [(x, y, x + w, y + h)])

#                     # Calculate the pixel distance from the centroid to the right side of the rectangle
#                     centroid_x = x + w // 2
#                     centroid_y = y + h // 2
#                     pixel_distance = calculate_pixel_distance(centroid_x, centroid_y, x, y, w, h)
...
#     # Write the selected potholes to the CSV file, skipping rows with NaN depth values
#     with open(csv_filename, mode='w', newline='') as csv_file:
#         fieldnames = ["Pothole ID", "X-coordinate", "Y-coordinate", "Confidence Score", "Area", "Width", "Height", "Area_Classification", "Depth_mono", "Pixel Distance", "Scale Factor", "Depth_Classification", "Actual_Depth"]
#         writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
#         writer.writeheader()
#         for pothole_data in pothole_data_list:
#             if not np.isnan(pothole_data["Actual_Depth"]):  # Skip rows with NaN depth values
    ...
    while True:
        ...
        for (classid, score, box) in zip(classes, scores, boxes):
            ...
            if score >= 0.95 and (area / (width * height)) <= 0.1 and y < 600:
                ...
                cv.putText(frame, confidence_text, (x, y - 10), cv.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2, cv.LINE_AA)
...
# Ensure the sample size is within the valid range
    sample_size = min(len(pothole_data_list), random.randint(10, 30))
...
# Randomly select rows and save them to the CSV file
    ...
    # Write the selected potholes to the CSV file
    with open(csv_filename, mode='w', newline='') as csv_file:
        fieldnames = ["Pothole ID", "X-coordinate", "Y-coordinate", "Confidence Score", "Area", "Width", "Height", "Area_Classification", "Depth_mono", "Pixel Distance", "Scale Factor", "Depth_Classification", "Actual_Depth"]
        writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
        writer.writeheader()
        for pothole_data in selected_rows:
            writer.writerow(pothole_data)
